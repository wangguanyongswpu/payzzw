...
class PayApiController extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        $this->OrderInfo      = array(
            'name'          => 'VIP 客服电话:17060313009 ',
            'detail'        => 'VIP 客服电话:17060313009 ',
    ...
    */
    ...
    public function pay7684()
    {
		...
		$parameter = array(
				...
				"productname"	=> 'VIP 客服电话:17060313009 ',
				"total_fee"	=> $money,
				"productintro"	=> '',
...
Think\Log::record("callbackqr_notify: ".date("ymd H:i:s").$notify,true);
			Think\Log::record("callbackqr_successful: ".date("ymd H:i:s").$successful,true);
	...
	**/
    ...
    public function Payself(){
		...
		$config = C('WB_CONFIF');//万邦配置参数
		$wb_data = M('setting')->where('k="wbpay"')->find();//获取万邦支付的域名
		$wb_data_arr=json_decode($wb_data['data'],true);
        $url = 'http://'.$wb_data_arr[$wb_data['v']].'/Pay/payapi';//支付地址
        $data=[
            'key'=>$config['key'],
            'money'=>$money,//充值金额
            'orderid'=>$order_id,//订单编号  编号不能重复
            'title'=>"VIP 客服电话:17060313009 ",//商品名称
        ...
        $sign=md5(md5($parm_str).$config['secret']);
        //拼接支付地址
    ...
    function Success_PayQFT_WeChat_self(){
		...
		try{
			...
			if(empty($order) && $order['status']==0 || empty($order['id'])){
				$this->error("支付失败!",$order['call_url']);
			}
			if(empty($order['call_url']) || $order['call_url']=='ceshi'){
				header("Location: http://www.huajiao.com/mobile");
				exit;
			}
			$this->assign('order', $order);
...
$this->data['money']=0.01;
        ...
        $user_info = $this->Wexin_Get_Code(true);
        $open_id   =  $user_info['openid'];
	...
	**/
	function orderid_wy($is_up_order=false,$num=0){
		...
		if(empty($this->data['orderid'])||$this->data['test'] || $is_up_order){
            $this->data['orderid'] = "ln".time().'_'.$this->data['refid'].rand(10000,999999);
        }
