...
Class Request{
    ...
    public function Request($service){
        ...
        empty($this->service) && $this->service = 'js';
     ...
     * @return array
     */
    public function submitOrderInfo($param = array()){
        $this->reqHandler->setReqParams($param['req']);
        $this->reqHandler->setParameter('service',$param['service']);//接口类型：pay.weixin.wappay
        ...
        //$this->reqHandler->setParameter('notify_url',$notify_url.'/payInterface/Wechatwap.php?method=callback');
		$this->reqHandler->setParameter('notify_url',$param['nurl']);//通知回调地址，目前默认是空格，商户在测试支付和上线时必须改为自己的，且保证外网能访问到
		$this->reqHandler->setParameter('callback_url',$param['aurl']);
        $this->reqHandler->setParameter('nonce_str',mt_rand(time(),time()+rand()));//随机字符串，必填项，不长于 32 位
        if($param['sub_appid']){
            $this->reqHandler->setParameter('sub_appid',$param['sub_appid']);
        }
        ...
        if($this->pay->call()){
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    $res = array('pay_info'=>$this->resHandler->getParameter('pay_info'));
                    if($this->service == 'js'){
                        $res = array('token_id'=>$this->resHandler->getParameter('token_id'));
                    }
                    return $res;
                }else{
    ...
    public function callback(){
        ...
        if($this->resHandler->isTenpaySign()){
            if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
				...
				//更改订单状态
				return array('status'=>'00', 'msg'=> '成功', 'data'=>$this->resHandler->getAllParameters());
            }else{
                return array('status'=>'10', 'msg'=> '支付失败', 'data'=>$this->resHandler->getAllParameters());
            }
        }else{
            return array('status'=>'20', 'msg'=> '签名错误', 'data'=>$this->resHandler->getAllParameters());
        }
    }
}